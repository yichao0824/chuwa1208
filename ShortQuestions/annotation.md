...
`@Aspect`
Marks a class as an aspect that contains cross-cutting concerns.

`@Pointcut`
Defines a reusable pointcut expression that matches join points.

`@Before`
Executes advice before the target method is invoked.

`@After`
Executes advice after the target method completes, regardless of its outcome.

`@AfterReturning`
Executes advice after the target method successfully returns a value.

`@AfterThrowing`
Executes advice if the target method throws an exception.

`@Around`
Wraps the target method execution and allows full control over its invocation.

`@JoinPoint`
Provides reflective access to the details of the current method invocation.

`@ProceedingJoinPoint`
Extends `@JoinPoint` and allows the target method to be executed explicitly.
